...
function M.update(status_message, game_state, level_state, player_state, enemies_state, num_values, last_reward) -- Note: Original didn't pass total_bytes_sent
    ...
    for i = 1, 16 do -- Assuming 16 segments for fuseball charging
        table.insert(charging_fuseball_parts, (enemies_state.charging_fuseball_segments and enemies_state.charging_fuseball_segments[i] == 1) and "*" or "-")
    end
    add_custom_line("Fuseball Chrg: " .. table.concat(charging_fuseball_parts, " "))
    add_blank_line()
