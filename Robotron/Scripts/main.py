...
def _resolve_dashboard_host() -> str:
    explicit = os.getenv("ROBOTRON_DASHBOARD_HOST", "").strip()
    if explicit:
        return explicit
...
def print_network_info(agent):
    print("\n" + "=" * 90)
    print("ROBOTRON AI v2 — Rainbow Engine".center(90))
    print("=" * 90)
...
def main():
    ...
    if os.getenv("ROBOTRON_DASHBOARD_BROWSER") is None:
